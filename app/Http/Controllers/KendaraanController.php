...
class KendaraanController extends Controller
{
    ...
    public function kendaraandata(Request $request)
    {
        ...
        $dtKendaraans = $kendaraans->paginate(5);
...
//            ->get();
    ...
    public function kendaraanedit(Request $request)
    {
        $kode_kendaraan = $request->kode_kendaraan;
        //return $kode_kendaraan;
        $kendaraan = DB::table('kendaraan')->where('kode_kendaraan', $kode_kendaraan)->first();
        return view('kendaraan.edit', compact('kendaraan'));

    }

    public function kendaraanupdate(Request $request, $kode_kendaraan)
    {
        //return $kode_kendaraan;
        $kode_kendaraan_edit = $request->kode_kendaraan_edit;
        $nama_kendaraan_edit = $request->nama_kendaraan_edit;
        $plat_kendaraan_edit = $request->plat_kendaraan_edit;
        $jenis_kendaraan_edit = $request->jenis_kendaraan_edit;
        $old_foto_kendaraan_edit = $request->old_foto_kendaraan_edit;

        if ($request->hasFile('foto_kendaraan_edit')) {
            $foto_edit = $kode_kendaraan_edit . "." . $request->file('foto_kendaraan_edit')->getClientOriginalExtension();
        } else {
            $foto_edit = $old_foto_kendaraan_edit;
        }

        $get_data_old = DB::table('kendaraan')->where('kode_kendaraan', $kode_kendaraan_edit)->first();
        $get_old_plat_kendaraan = $get_data_old->plat_kendaraan;
        $simpan = "n";

        if ($get_old_plat_kendaraan == $request->plat_kendaraan_edit) {
            $plat_kendaraan_edit = $request->plat_kendaraan_edit;
            $simpan = "y";
        } else if ($get_old_plat_kendaraan != $request->plat_kendaraan_edit) {
            $plat_kendaraan_edit = $request->plat_kendaraan_edit;
            $cek_kendaraan = DB::table('kendaraan')
                ->where('plat_kendaraan', $plat_kendaraan_edit);
            if ($cek_kendaraan->count() > 0) {
                $simpan = "n";
            } else if ($cek_kendaraan->count() <= 0) {
                $simpan = "y";
            }
        }
        if ($simpan == "n") {
            return redirect()->route('kendaraandata')->with(['warning' => 'Plat Kendaraan telah terdaftar']);
        } else if ($simpan == "y") {
            try {
                $data = [
                    "name_kendaraan" => $nama_kendaraan_edit,
                    "plat_kendaraan" => $plat_kendaraan_edit,
                    "jenis_kendaraan" => $jenis_kendaraan_edit,
                    "foto" => $foto_edit,
                    "updated_at" => date('Y-m-d H:i:s'),
                ];
                $update = DB::table('kendaraan')->where('kode_kendaraan', $kode_kendaraan_edit)->update($data);
                if ($update) {
                    if ($request->hasFile('foto_kendaraan_edit')) {
                        $folderPath = "public/uploads/salaman";
                        $folderPathOld = "public/uploads/salaman/" . $old_foto_kendaraan_edit;

                        /*hapus dulu foto lama*/
                        Storage::delete($folderPathOld);

                        /*upload kemudian foto baru*/
                        $request->file('foto_kendaraan_edit')->storeAs($folderPath, $foto_edit);
                    }
                    return redirect()->route('kendaraandata')->with(['success' => 'Data Kendaraan Berhasil di-Update']);
                } else {
                    return redirect()->route('kendaraandata')->with(['warning' => 'Data Kendaraan Gagal di-Update']);
                }
            } catch (Exception $e) {
                return redirect()->route('kendaraandata')->with(['warning' => 'Data Kendaraan Gagal di-Update, error : '.$e]);
            }
        }

    }

    public function kendaraandelete($kode_kendaraan){
        $kendaraan = DB::table('kendaraan')->where('kode_kendaraan', $kode_kendaraan)->first();
        $foto_to_delete = $kendaraan->foto;
        $folderPathOld = "public/uploads/salaman/" . $foto_to_delete;
        $delete = DB::table('kendaraan')->where('kode_kendaraan', $kode_kendaraan)->delete();
        if ($delete) {
            Storage::delete($folderPathOld);
            return redirect()->route('kendaraandata')->with(['success' => 'Data Berhasil Dihapus!']);
        } else {
            return redirect()->route('kendaraandata')->with(['warning' => 'Data Gagal Dihapus!']);
        }
    }
